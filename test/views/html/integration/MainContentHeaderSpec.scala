  ...
  "Rendering mainContentHeader.scala.html" must {
    ...
    "show last logged in details with name when a name is present and a lastLogin is supplied" in {
      ...
      val document = asDocument(
        view(Some("Firstname"), Some(millis), Nil, false, None, None).toString
      )
      document
        .select(".last-login > p")
        .text mustBe "Firstname, you last signed in 12:00am, Friday 30 April 1982"
    }

    "show last logged in details without name when no name is present and a lastLogin is supplied" in {
      val millis = DateTime.parse("1982-04-30T00:00:00.000+01:00")
      val document =
        asDocument(view(None, Some(millis), Nil, false, None, None).toString)
      document
        .select(".last-login > p")
        .text mustBe "You last signed in 12:00am, Friday 30 April 1982"
    }

    "not show last logged in details when lastLogin is not supplied" in {
      val document =
        asDocument(view(None, None, Nil, false, None, None).toString)
    ...
    "hide breadcrumb when none is passed" in {
      val document =
        asDocument(view(None, None, Nil, true, None, None).toString)
    ...
    "show feedback link in BETA banner when passed deskProToken with PTA" in {
      val document =
        asDocument(view(None, None, Nil, true, Some("PTA"), None).toString)
      document
