...
//        when(mockConfigDecorator.updateInternationalAddressInPta) thenReturn false
  ...
  "Calling AddressController.processAddressSelectorForm" should {
    ...
    "call the address lookup service and return 400 when supplied no addressId in the form" in new LocalSetup {
      ...
      val result = c1.processAddressSelectorForm(PostalAddrType, None)(form)
...
//
//      val result = controller.closePostalAddressChoice(buildFakeRequestWithAuth("GET"))
//
//      status(result) shouldBe OK
//    }
//  }
...
//      val result = controller.processClosePostalAddressChoice(
...
//
//    "return a bad request when supplied no value" in new LocalSetup {
//      val result = controller.processClosePostalAddressChoice(buildFakeRequestWithAuth("POST"))
//
//      status(result) shouldBe BAD_REQUEST
//    }
//  }
...
//
//      val result = controller.confirmClosePostalAddress(buildFakeRequestWithAuth("GET"))
//
//      status(result) shouldBe OK
//    }
//  }
...
//      val result = controller.submitConfirmClosePostalAddress(FakeRequest("POST", ""))
//
//      status(result) shouldBe OK
//
...
//          )))
//
//      val result = controller.submitChanges(PrimaryAddrType)(FakeRequest("POST", ""))
//
//      status(result) shouldBe SEE_OTHER
//      redirectLocation(result) shouldBe Some("/personal-account/personal-details")
//
...
//          )))
//
//      val result = controller.submitChanges(SoleAddrType)(FakeRequest("POST", ""))
//
//      status(result) shouldBe SEE_OTHER
//      redirectLocation(result) shouldBe Some("/personal-account/personal-details")
//
...
//          )))
//
//      val result = controller.submitChanges(PostalAddrType)(FakeRequest("POST", ""))
//
//      status(result) shouldBe OK
//      verify(controller.sessionCache, times(1)).fetch()(any(), any())
//      verify(mockCitizenDetailsService, times(1)).updateAddress(meq(nino), meq("115"), meq(fakeAddress))(any())
...
//        ))
//
//      val result = controller.submitChanges(PostalAddrType)(
...
//      }
//
//      lazy val result = c1.lookingUpAddress(SoleAddrType, "AA1 1AA", false) {
