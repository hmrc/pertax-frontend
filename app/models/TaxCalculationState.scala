...
case class TaxCalculationOverpaidRefundState(
  amount: BigDecimal,
  startOfTaxYear: Int,
  endOfTaxYear: Int
...
case class TaxCalculationOverpaidPaymentChequeSentState(
  amount: BigDecimal,
  datePaid: Option[LocalDate]
) extends TaxCalculationOverpaidState
...
case class TaxCalculationUnderpaidPaidAllState(
  startOfTaxYear: Int,
  endOfTaxYear: Int,
  dueDate: Option[LocalDate]
) extends TaxCalculationUnderpaidState
case class TaxCalculationUnderpaidPaymentsDownState(
  startOfTaxYear: Int,
  endOfTaxYear: Int
) extends TaxCalculationUnderpaidState
  ...
  def buildFromTaxCalculation(
    ...
    (taxCalculation, includeOverPaidPayments) match {
      ...
      case (
        ...
        TaxCalculationUnderpaidPaidAllState(
          taxYear,
          taxYear + 1,
          Some(new LocalDate(dueDate))
        )
      ...
      case (
        ...
        TaxCalculationOverpaidPaymentPaidState(
          amount,
          Some(new LocalDate(datePaid))
        )
      ...
      case (
        ...
        TaxCalculationOverpaidPaymentChequeSentState(
          amount,
          Some(new LocalDate(datePaid))
        )
...
object SaDeadlineStatusCalculator {
  ...
  def getSaDeadlineStatus(
    dueDate: LocalDate
  )(implicit configDecorator: ConfigDecorator): Option[SaDeadlineStatus] = {

    val now = new LocalDate(configDecorator.currentLocalDate)
    val dueDateEquals31stJanuary =
      dueDate.getMonthOfYear == 1 && dueDate.getDayOfMonth == 31
    val dueDatePassed = now.isAfter(dueDate)
    val datePassed14thDec =
      now.isAfter(
        new LocalDate(
          taxYearFor(configDecorator.currentLocalDate).currentYear + "-12-14"
        )
      )
    val dateWithin30DaysOfDueDate = now.isAfter(dueDate.minusDays(31))

    (
      dueDateEquals31stJanuary,
      dueDatePassed,
      datePassed14thDec,
      dateWithin30DaysOfDueDate
    ) match {
