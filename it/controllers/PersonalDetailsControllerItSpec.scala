...
"/personal-account/your-profile" must {
    ...
    val designatoryDetailsUrl = s" /citizen-details/$generatedNino/designatory-details"

    "return an OK response and contain a link to TCS address change if the user has tax credits" in {

      server.stubFor(
        get(urlEqualTo(tcsBrokerUrl))
          .willReturn(ok(FileHelper.loadFile("./it/resources/dashboard-data.json")).withStatus(OK))
      )
