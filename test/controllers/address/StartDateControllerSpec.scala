...
class StartDateControllerSpec extends BaseSpec with CitizenDetailsFixtures {
  ...
  private val submittedUkAddress: AddressDto =
    asAddressDto(fakeStreetTupleListAddressForUnmodified).copy(postcode = Some("AA1 1AA"))
    ...
    override def authWithPersonalDetails: ActionBuilder[UserRequest, AnyContent] =
      new ActionBuilder[UserRequest, AnyContent] {
        override def parser: BodyParser[AnyContent] =
          play.api.test.Helpers.stubBodyParser()
  ...
  private lazy val controller: StartDateController =
    app.injector.instanceOf[StartDateController]
  ...
  "onPageLoad" must {
    ...
    "return 200 when passed ResidentialAddrType and submittedAddress is in cache" in {
      ...
      val result: Future[Result] =
        controller.onPageLoad(ResidentialAddrType)(currentRequest)
    ...
    "redirect to 'edit address' when passed PostalAddrType as this step is not valid for postal" in {
      ...
      val result: Future[Result] =
        controller.onPageLoad(PostalAddrType)(currentRequest)
  ...
  "onSubmit" must {
    ...
    "return 400 with P85 messaging when passed ResidentialAddrType and the updated start date is not after the start date on record (international address)" in {
      val address       =
        buildPersonDetailsCorrespondenceAddress.address.map(_.copy(startDate = Some(LocalDate.of(2016, 11, 22))))
      ...
      when(mockAddressCountryService.deriveCountryForPostcode(any())(any()))
        .thenReturn(Future.successful(Some("FR")), Future.successful(Some("FR")))
      ...
      status(result) mustBe BAD_REQUEST
      contentAsString(result) must include("You will need to complete a P85 form (opens in new tab)")
    }
    ...
    "redirect (no P85 messaging) when startDate is earlier than recorded with residential address type (domestic, non cross-border)" in {
      val submittedAddress: AddressDto =
        asAddressDtoWithSubdivision(fakeStreetTupleListAddressForUnmodified, "GB-ENG").copy(postcode = Some("AA1 1AA"))
