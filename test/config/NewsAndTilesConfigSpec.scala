  ...
  "getNewsAndContentModelList" must {
    "read configuration and create a list ordered by recency" in {
      ...
      val app = localGuiceApplicationBuilder()
        .configure(
          "feature.override-start-and-end-dates-for-news-items.enabled" -> false,
          "feature.news.0.name"                                         -> "childBenefits",
          "feature.news.0.start-date"                                   -> LocalDate.now().format(formatter),
          "feature.news.0.end-date"                                     -> LocalDate.now().plusYears(1).format(formatter),
          "feature.news.0.dynamic-content"                              -> true,
          "feature.news.1.name"                                         -> "hmrcApp",
          "feature.news.1.start-date"                                   -> LocalDate.now().minusWeeks(2).format(formatter),
          "feature.news.1.dynamic-content"                              -> true,
          "feature.news.2.name"                                         -> "payeEmployments",
          "feature.news.2.start-date"                                   -> LocalDate.now().minusWeeks(1).format(formatter),
          "feature.news.2.end-date"                                     -> LocalDate.now().plusYears(1).format(formatter),
          "feature.news.2.dynamic-content"                              -> true,
          "play.cache.bindCaches"                                       -> List("controller-cache", "document-cache"),
          "play.cache.createBoundCaches"                                -> false
        )
        .build()
