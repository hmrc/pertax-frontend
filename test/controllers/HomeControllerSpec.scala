...
class HomeControllerSpec extends BaseSpec with CurrentTaxYear {
  ...
  val mockIdentityVerificationFrontendService =
  ...
  trait LocalSetup {
    ...
    lazy val authProviderType: String =
      UserDetails.GovernmentGatewayAuthProvider
    lazy val nino: Nino = Fixtures.fakeNino
    lazy val personDetailsResponse: PersonDetailsResponse =
      PersonDetailsSuccessResponse(Fixtures.buildPersonDetails)
    lazy val confidenceLevel: ConfidenceLevel = ConfidenceLevel.L200
    lazy val withPaye: Boolean = true
    lazy val year = 2017
    lazy val getTaxCalculationResponse: TaxCalculationResponse =
      TaxCalculationSuccessResponse(
        TaxCalculation(
          "Overpaid",
          BigDecimal(84.23),
          2015,
    ...
    lazy val getPaperlessPreferenceResponse: ActivatePaperlessResponse =
      ActivatePaperlessActivatedResponse
    lazy val getIVJourneyStatusResponse: IdentityVerificationResponse =
      IdentityVerificationSuccessResponse("Success")
    lazy val getCitizenDetailsResponse = true
    lazy val selfAssessmentUserType: SelfAssessmentUserType =
    ...
    when(
      mockTaiService
        .taxComponents(any[Nino](), any[Int]())(any[HeaderCarrier]())
    ) thenReturn {
      Future.successful(TaxComponentsSuccessResponse(buildTaxComponents))
    }
    when(
      mockTaxCalculationService
        .getTaxYearReconciliations(any[Nino])(any[HeaderCarrier])
    ) thenReturn {
      Future.successful(buildTaxYearReconciliations)
    }
    when(
      mockPreferencesFrontendService.getPaperlessPreference()(any())
    ) thenReturn {
      Future.successful(getPaperlessPreferenceResponse)
    }
    when(
      mockIdentityVerificationFrontendService.getIVJourneyStatus(any())(any())
    ) thenReturn {
    ...
    when(mockLocalSessionCache.fetch()(any(), any())) thenReturn {
      Future.successful(
        Some(CacheMap("id", Map("urBannerDismissed" -> JsBoolean(true))))
      )
    }
    ...
    when(
      mockConfigDecorator.allowLowConfidenceSAEnabled
    ) thenReturn allowLowConfidenceSA
    when(
      mockConfigDecorator.identityVerificationUpliftUrl
    ) thenReturn "/mdtp/uplift"
    ...
    when(
      mockConfigDecorator.citizenAuthFrontendSignOut
    ) thenReturn "/ida/signout"
    when(mockConfigDecorator.defaultOrigin) thenReturn Origin("PERTAX")
    when(
      mockConfigDecorator.getFeedbackSurveyUrl(Origin("PERTAX"))
    ) thenReturn "/feedback/PERTAX"
    when(
  ...
  "Calling HomeController.index" must {
    ...
    "return a 200 status when accessing index page with good nino and sa User" in new LocalSetup {
      ...
      if (config.taxComponentsEnabled)
        verify(controller.taiService, times(1)).taxComponents(
          meq(Fixtures.fakeNino),
          meq(current.currentYear)
        )(any())
      if (config.taxcalcEnabled)
        verify(controller.taxCalculationService, times(1))
          .getTaxYearReconciliations(meq(Fixtures.fakeNino))(any())
    }
    ...
    "return 200 when TaxCalculationService returns TaxCalculationNotFoundResponse" in new LocalSetup {
      ...
      override lazy val getTaxCalculationResponse =
  ...
  "Calling serviceCallResponses" must {
    ...
    "return TaxComponentsDisabled where taxComponents is not enabled" in new LocalSetup {
      ...
      val (result, _, _) =
        await(controller.serviceCallResponses(userNino, year))
    ...
    "return TaxComponentsNotAvailableState status when TaxComponentsUnavailableResponse from TaxComponents" in new LocalSetup {
      ...
      when(
        mockTaiService.taxComponents(any[Nino], any[Int])(any[HeaderCarrier])
      ) thenReturn {
    ...
    "return TaxComponentsUnreachableState status when there is TaxComponents returns an unexpected response" in new LocalSetup {
      ...
      when(
        ...
        Future.successful(
          TaxComponentsUnexpectedResponse(HttpResponse(INTERNAL_SERVER_ERROR))
        )
      }
    ...
    "return only  CY-1 None and CY-2 None when get TaxYearReconcillation returns Nil" in new LocalSetup {
      ...
      when(
        mockTaxCalculationService.getTaxYearReconciliations(any())(any())
