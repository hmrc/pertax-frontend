...
class homeSpec extends BaseSpec {
  ...
  "Rendering home.scala.html" should {
    ...
    "not show 'Check your tax codes and an estimate of the Income Tax you'll pay.' for the PAYE section when the user has details but is not active PAYE" in new SpecSetup {
      override val withPaye: Boolean = true
      override val withActivePaye = false
      override val withSa: Boolean = false
      override val isGovernmentGateway: Boolean = false
      override val isHighGG: Boolean = true
      override val principalName: Option[String] = None
    ...
    "show an SA Messages link when a SA, GG user is logged in" in new SpecSetup {
      ...
      override lazy val document: Document = Jsoup.parse(views.html.home(Html("""<div class="pertax-messages">Simulated Messages Partial</div>"""),
        showMarriageAllowanceSection = true,
        isActivePaye = withActivePaye,
        showCompanyBenefitSection = true,
        taxCalculationState = TaxCalculationUnkownState,
    ...
    "show an SA link when a GG and SA user is logged in" in new SpecSetup {
      ...
      override lazy val document: Document = Jsoup.parse(views.html.home(Html(""),
        showMarriageAllowanceSection = true,
        isActivePaye = true,
        showCompanyBenefitSection = true,
        taxCalculationState = TaxCalculationUnkownState,
        saActionNeeded = FileReturnSelfAssessmentActionNeeded(SaUtr("1111111111")),
        showLtaSection = false,
        userResearchLinkUrl = None)
      (PertaxContext(FakeRequest("GET", "/"),
        mockLocalPartialRetreiver,
    ...
    "not show the Marriage Allowance link when user is low GG but not uplifted and showMarriageAllowanceSection is false" in new SpecSetup {
      ...
      override lazy val document: Document = Jsoup.parse(views.html.home(Html(""),
        showMarriageAllowanceSection = false,
        isActivePaye = withActivePaye,
        showCompanyBenefitSection = true,
        taxCalculationState = TaxCalculationUnkownState,
        saActionNeeded = NoSelfAssessmentActionNeeded,
    ...
    abstract class WithIncomeTaxBlocksSetup {
      ...
      lazy val pertaxUser: PertaxUser = Fixtures.buildFakePertaxUser(withPaye = withPaye, withSa = withSa, isGovernmentGateway = isGovernmentGateway)
      lazy val document: Document = Jsoup.parse(views.html.home(
      ...
      def runCheck(saBlockExpected: Boolean, saMessage: String, payeBlockExpected: Boolean, incomeTaxHeaderExpected: Boolean): Assertion = {
    ...
    abstract class WithNationalInsuranceBlocksSetup {
      ...
      lazy val pertaxUser: PertaxUser = Fixtures.buildFakePertaxUser(withPaye = withPaye, withSa = withSa, isGovernmentGateway = isGovernmentGateway, isHighGG = isHighGG)
      lazy val document: Document = Jsoup.parse(views.html.home(Html(""),
        showMarriageAllowanceSection = true,
        isActivePaye = withActivePaye,
        showCompanyBenefitSection = true,
        taxCalculationState = TaxCalculationUnkownState,
      ...
      def runCheck(niBlockExpected: Boolean, companyBenefitsBlockExpected: Boolean): Assertion = {
    ...
    abstract class WithTaxAndChildBenefitsBlocksSetup {
      ...
      def runCheck(taxCreditsBlockExpected: Boolean, childBenefitsBlockExpected: Boolean): Assertion = {
    ...
    abstract class WithLtaBlocksSetup {
      ...
      def runCheck(lifetimeAllowanceProtectionBlockExpected: Boolean): Assertion = {
  ...
  "Should see 'self assessment bucket' on the homepage with activation code instructions as a GG User who has received their 12 digit activation code" in new OldStyleSpecSetup {
    val pertaxUser: PertaxUser = Fixtures.buildFakePertaxUser(withSa = true, isGovernmentGateway = true)
    val document: Document = Jsoup.parse(views.html.home(Html(""),
      showMarriageAllowanceSection = true,
      isActivePaye = true,
      showCompanyBenefitSection = true,
      taxCalculationState = TaxCalculationUnkownState,
      saActionNeeded = ActivateSelfAssessmentActionNeeded(SaUtr("1111111111")),
      showLtaSection = false,
      userResearchLinkUrl = None)
    (PertaxContext(FakeRequest("GET", "/"),
      mockLocalPartialRetreiver,
  ...
  "Should see 'self assessment bucket' on the homepage linking to 'sa-not-shown' as GG User who already has an SAUTR that exists in citizen details" in new OldStyleSpecSetup {
    ...
    val document: Document = Jsoup.parse(views.html.home(Html(""),
      ...
      showCompanyBenefitSection = true,
      taxCalculationState = TaxCalculationUnkownState,
      saActionNeeded = NoEnrolmentFoundSelfAssessmentActionNeeded(SaUtr("1111111111")),
      showLtaSection = false,
      userResearchLinkUrl = None)
    (PertaxContext(FakeRequest("GET", "/"),
      mockLocalPartialRetreiver,
