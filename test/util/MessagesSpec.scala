...
class MessagesSpec extends BaseSpec {
  ...
  "Application" must {
    ...
    "have messages for default and cy only" in {
      ...
      val englishMessageCount =
        messagesAPI.messages("default").size - frameworkProvidedKeys.size
  ...
  "All message files" must {
    ...
    "have a resolvable message for keys which take args" in {
      ...
      val missingFromEnglish =
        englishWithArgsMsgKeys.toList diff welshWithArgsMsgKeys.toList
      val missingFromWelsh =
        welshWithArgsMsgKeys.toList diff englishWithArgsMsgKeys.toList
  ...
  private def assertCorrectUseOfQuotes(
    label: String,
    messages: Map[String, String]
  ) =
    messages.foreach {
  ...
  private lazy val defaultMessages: Map[String, String] =
    getExpectedMessages("default") -- providedKeys
