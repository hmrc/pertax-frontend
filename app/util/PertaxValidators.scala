...
object PertaxValidators {
  ...
  def fieldsArePresentIfCurrentFieldIsMissingFormDataValidator(fieldsToCheck: String*)(key: String, formData: Map[String, String]): Seq[FormError] = {

    formData.getOrElse(key, "") match {
      case "" =>
        val anyCheckedFieldContainsData = fieldsToCheck.foldLeft[Boolean](false)((dataFound, rf) => dataFound || !formData.getOrElse(rf, "").isEmpty)
  ...
  def optionalTextIfFieldsHaveContent(requiredFields: String*) = optionalTextIfFormDataValidatesMapping(fieldsArePresentIfCurrentFieldIsMissingFormDataValidator(requiredFields: _*))

  def optionalTextIfFormDataValidatesMapping(formDataValidator: FormDataValidator): FieldMapping[Option[String]] = of(new Formatter[Option[String]] {
