...
class ErrorRenderer @Inject() (
  ...
  def futureError(statusCode: Int)(implicit request: UserRequest[?], messages: Messages): Future[Result] =
    Future.successful(error(statusCode))

  def error(statusCode: Int)(implicit request: WrappedRequest[?], messages: Messages): Result = {
