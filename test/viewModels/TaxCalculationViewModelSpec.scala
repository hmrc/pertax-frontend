...
class TaxCalculationViewModelSpec extends ViewSpec {
  ...
  def view(reconciliation: Reconciliation): Option[Document] =
    TaxCalculationViewModel
      .fromTaxYearReconciliation(TaxYearReconciliation(2017, reconciliation))(
        config
      )
      .map { taxRec =>
  ...
  def formatDate(date: LocalDate) =
    injected[LanguageUtils].Dates
      .formatDate(Some(date), "dd MMMM yyyy")(messages)
  ...
  "taxCalculation" should {
    ...
    "give the correct heading" when {
      ...
      "status is Underpaid PaymentDue and SA deadline has passed" in {
        ...
        val status =
          Underpaid(Some(100.00), Some(LocalDate.now.minusDays(1)), PaymentDue)
      ...
      "status is Underpaid PartPaid and SA deadline has passed" in {
        ...
        val status =
          Underpaid(Some(100.00), Some(LocalDate.now.minusDays(1)), PartPaid)
    ...
    "display the correct content" when {
      ...
      "status is UnderpaidPaymentDue" in {
        ...
        assertContainsText(
          view(status).getValue,
          messages("label.you_owe_hmrc", "100.00")
        )
        assertContainsLink(
          view(status).getValue,
