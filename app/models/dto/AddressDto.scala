...
case class AddressDto(
  ...
  def toList: Seq[String] =
    Seq(Some(line1), Some(line2), Some(line3), line4, line5, postcode).flatten

  def toListWithCountry: Seq[String] =
    Seq(Some(line1), Some(line2), Some(line3), line4, line5, country).flatten
...
object AddressDto extends CountryHelper {
  def unapply(obj: AddressDto): Some[
    (String, String, String, Option[String], Option[String], Option[String], Option[String], Option[String])
  ] = Some(
  ...
  def apply(
    ...
    List(line4, line5).filter(op => op.exists(line => line.nonEmpty)).padTo(2, None) match {
      case List(newLine4, newLine5) =>
        new AddressDto(line1, line2, line3, newLine4, newLine5, postcode, country, propertyRefNo)
    }
  ...
  val ukForm: Form[AddressDto] = Form(
    mapping(
      ...
      "line3"         -> text
        .verifying("error.line3_required", _.nonEmpty)
        .verifying("error.line3_contains_more_than_35_characters", _.length <= 35)
        .verifying("error.line3_invalid_characters", e => validateAddressLineCharacters(Some(e))),
      "line4"         -> optional(text)
